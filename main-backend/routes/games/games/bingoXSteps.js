//imports
let calculateWinningBingoX = require('./bingoXCalculation.js');
const { Interface } = require("./interface.js");

const Players = require('../../../models/Players.js');
const BingoXRounds = require('../../../models/BingoXRounds.js');
...
class BingoX {
	...
	constructor() {
		return(async() => {
			...
			let interface2 = await new Interface("bingoX", this.dataFromPlayerHandler);
	...
	async _onNewPlayer(userKey) {
		...
		let round = await BingoXRounds.find({num: {$exists: true}}).limit(25).sort({_id: -1});
		let nums = [];
	...
	async _onSendBets(userKey, data) {
		...
		let round = await BingoXRounds.findOne().sort({_id: -1});
		if (userKey === 1) {
			...
			await BingoXRounds.updateOne({_id: this.currentRoundID}, {$set: {bets: bets}}, {upsert: false});
			await new GameLogBot({gameName: "bingoX", bets: data.bets, price: +price, roundId: round._id.toString()}).save();
		}
		...
		if((user) && (!data.demo)) {
			...
			if(price <= allBalance) {
				...
				await GamesLog({gameName: "bingoX", player: user.idUser.toString(), amount: price, type: "bet", currentRoundID: this.currentRoundID.toString()}).save();
			}  
		} else {
			// it's not user, it's demo-player
			let userDemo = await BingoXDemo.findOne({userKey: userKey});
	...
	async drawingStarts() {
		...
		if (!num || num === false) {
			...
			for (let i = 0; i < round.bets.length; i++) {
				let userDemo = await BingoXDemo.findOne({userKey: round.bets[i].userKey});
				if (userDemo) return;
		...
		this.lastNum = +num+1;
		await BingoXRounds.updateOne({_id: this.currentRoundID}, {$set: {num: this.lastNum}}, {upsert: false});
		...
		let round = await BingoXRounds.findOne({}).sort({_id: -1});
		this.currentRoundID = await BingoXRounds({}).save();

		// generate list of winners
		let winArray = [];
		if (!round.bets) return;
		for (let i = 0; i < round.bets.length; i++) {
			let win = calculateWinningBingoX(round.bets[i].bets, this.lastNum);
		...
		for (let i = 0; i < winArray.length; i++) {
			if (!winArray[i].demo) {
				...
				if (!player) return;
				await GamesLog({gameName: "bingoX", player: player.idUser, amount: +winArray[i].win, type: "win", currentRoundID: round._id.toString()}).save();
