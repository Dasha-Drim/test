...
module.exports = async (idUser, amount) => {
  ...
  await new PaymentsLog({method: "cash", operationId: +countOperation+1, amount: +amount, currency: player.currency, type: "down", dateTime: DateTime.local().setZone("Europe/Moscow").toISO(), userId: player.idUser, status: "success"}).save();
