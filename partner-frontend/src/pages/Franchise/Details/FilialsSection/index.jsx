	...
	*/
	...
	return (
		<div className="FilialsSection">
			...
			{(props.id && role === "franchisee") && <AddButton to="add-filial" state={{idFranchisee: props.id}} name="Создать подразделение" />}
					...
					<tbody>
						{props.filials ? props.filials.map((item, key) =>
							<tr key={key}>
								...
								<td>{item.profit}</td>
								<td>
									<DetailsLinkMini to="/filial" state={{idFilial: item.idFilial, breadcrump: "Филиал " + item.name}} className="me-2" />
									{(role === "franchisee") && <TrashButtonMini onClick={() => deleteItem(item.idFilial)} />}
								</td>
							</tr>
						) :
							<tr>
